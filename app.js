...
function generatePDFReport(data) {
    ...
    img.onload = () => {
        ...
        const textData = [
            { text: data.Name, x: 110, y: 58, fontSize: 12, color: [1, 1, 1] },
            { text: data.Date_of_Birth, x: 110, y: 65, fontSize: 12, color: [1, 1, 1] },
            { text: data.CF, x: 110, y: 71, fontSize: 12, color: [1, 1, 1] },
            { text: data.Chronological_Age, x: 50, y: 71, fontSize: 12, color: [1, 1, 1] }
        ];
    
        textData.forEach(item => {
            pdf.setFontSize(item.fontSize); // Set font size
            pdf.setTextColor(...item.color); // Set text color
            pdf.text(item.text, item.x, item.y); // Add the text
        });
